...
# ---------------------------------------------------------------------------
# Dual-layer chronology: G_true vs G_perceived.
#
# The podcast is presented in an almost-linear investigation order, but real
# reporting doesn't move in lockstep with runtime -- some episodes cover a
# single tense afternoon, others skip weeks -- so true_time is perceived
# position plus small deterministic pacing jitter, not a bare 1:1 copy of it.
# A handful of beats are genuine flashbacks: the audience is shown something
# that happened chronologically much earlier than where it's placed in the
# season. Ep 22 is the explicit case the manifest names (twist-03) -- Tara's
# present-tense telling of the night before the Konkan Rani sailed, which
# actually happened in August 2009, years before the investigation starts.
# ---------------------------------------------------------------------------

# episode -> true_time override for beats whose depicted content is
# chronologically displaced from where the audience hears it.
TRUE_TIME_OVERRIDES: dict[int, float] = {
    22: 0.02,  # Tara's flashback: the night before the Rani sailed, 2009.
}

# Hand-set valence for the 35 hand-authored beats, on the -1..1 scale, chosen
# to match the emotional register of the prose above -- not derived from the
# text at inference time (that would violate the no-prose-features rule), but
# fixed ground truth baked into the generated data, same as everything else in
# SPECIAL_EPISODES.
SPECIAL_VALENCE: dict[int, float] = {
    1: 0.35,     # certainty, warmth, opens on trust in the tape.
    2: 0.05,     # quiet, ambiguous grief.
    3: -0.15,    # Tara's admission is uneasy.
    5: 0.2,      # Rao's offer, guarded hope.
    12: -0.5,    # the deckhand's account is harrowing.
    22: -0.1,    # unsettled, watched by Asha without correction.
    30: 0.15,    # another cautious promise.
    34: 0.1,     # a friendly introduction.
    40: 0.3,     # a kept plan, easy trust.
    47: -0.75,   # the fire; the season's most vivid trauma beat.
    52: -0.3,    # a small, nagging wrongness.
    55: 0.4,     # a promise paid, relief.
    60: -0.2,    # danger, tension of the dive.
    66: -0.1,    # guarded, suspicious first contact.
    71: 0.05,    # a conditional promise.
    88: -0.6,    # the second survivor's account, bleaker than the first.
    90: -0.2,    # the appraisal undercuts Asha's certainty.
    101: -0.15,  # the name slip, quietly wrong.
    110: -0.25,  # the first crack in the tape's authenticity.
    120: 0.2,    # another promise, procedural and calm.
    133: 0.35,   # kept exactly as promised.
    134: -0.4,   # the twin reveal, disorienting.
    140: 0.1,    # bittersweet retirement.
    160: 0.3,    # the raid succeeds, adrenaline.
    175: -0.1,   # an old wound, matter-of-fact.
    178: 0.15,   # a gap finally closed, gentle relief.
    180: 0.1,    # a promise with a deadline.
    190: -0.55,  # the scar has moved arms -- something is badly wrong.
    195: 0.3,    # Zoya keeps her word.
    199: -0.3,   # a heavy, complicated revelation.
    205: 0.1,    # bureaucratic, mildly hopeful.
    210: -0.85,  # the cassette's betrayal; the season's low point.
    212: 0.25,   # Rao's last promise, sincere.
    218: -0.35,  # Asha's own false memory, unsettling self-reckoning.
    220: 0.2,    # the finale: honest, unresolved, quietly hopeful.
}


def _pacing_jitter(episode: int) -> float:
    """Small deterministic pacing noise, independent of the filler RNG stream.

    Uses its own seed so it never perturbs the beat/excerpt text generated by
    ``generate_filler`` -- regenerating stays byte-identical for prose either
    way.
    """
    rng = random.Random(episode * 7919 + 13)
    return rng.uniform(-0.012, 0.012)


def _filler_valence(episode: int) -> float:
    """Deterministic background valence for episodes with no hand-authored tone."""
    rng = random.Random(episode * 104729 + 31)
    return round(rng.uniform(-0.6, 0.6), 3)
...
def build_nodes(content: dict[int, dict[str, object]]) -> list[dict[str, object]]:
    ...
    for episode in range(1, TOTAL_EPISODES + 1):
        ...
        base_true_time = (episode - 1) / (TOTAL_EPISODES - 1)
        true_time = TRUE_TIME_OVERRIDES.get(
            episode, max(0.0, min(1.0, base_true_time + _pacing_jitter(episode)))
        )
        valence = SPECIAL_VALENCE.get(episode, _filler_valence(episode))
